...
- Optional Special Powers mode: toggle on the setup page, pick one of 10 powers (Telepathy, Time Traveler, Luck of the Draw, Cold as Ice, Touch of Death, Greasy Fingers, Battery Powered, X-ray Vision, Electric Monkey, Sleight of Hand) via a swipeable selector; each power hooks into mismatch/match/flip events to alter scoring, timer, board contents, or card state, per session 10.2 and its per-power sub-sessions.
...
10. **10.2-powers-icons-and-plumbing.md** — Procedurally generated icons for all 10 powers, real registry entries/copy, and shared plumbing every power needs: score multiplier, timer adjustment, board pair-injection/burn/peek, frozen/paralyzed/highlighted card state, repeat-tap-arm detection, charge-based HUD buttons, forced-loss path. No power behavior yet — each registry entry still points at `NoOpPower`.
...
10. **10.2.2-power-time-traveler.md** — Time Traveler: every match adds 5s to the clock; 3 consecutive mismatches costs 2s.
...
10. **10.2.4-power-cold-as-ice.md** — Cold as Ice: every 3rd move (while nothing is frozen) freezes a random card face-up; left unmatched, an additional card locks face-down every 3 moves after that, stacking until any match breaks the ice.
...
10. **10.2.7-power-battery-powered.md** — Battery Powered: tapping your face-up first-pick card again supercharges it — a supercharged match pays 2x, a supercharged mismatch costs 50 points.
10. **10.2.8-power-xray-vision.md** — X-ray Vision: 2-charge button flashes the board face-up briefly; 3 mismatches after the first use burns a pair and costs 10s, 1 mismatch after the second use burns a pair and costs 20s + 100pts.
10. **10.2.9-power-electric-monkey.md** — Electric Monkey: tapping your face-up first-pick card twice charges it — a charged match zaps a random other pair for half points + 10s, a charged mismatch paralyzes 2 random cards.
10. **10.2.10-power-sleight-of-hand.md** — Sleight of Hand: 2-charge button secretly rescues your next mismatch into a match; the swapped-out card is marked red — match it later for 2x points, mismatch it and you instantly lose.
10. **10.3-leaderboard-improvements.md** — Richer leaderboard entries (date, time taken, moves, highest single-match points saved per entry); tap an entry for a detail view; Regular vs. Special Powers toggle; per-power filter chips and a small power icon shown on each entry.
10. **10.4-scoring-improvements.md** — Move-threshold bonus multiplier applied at game end (easy/medium/hard thresholds 20/40/60; ≤half-threshold = flat 2x; linear taper from 2x to 1x between half-threshold and threshold; at/above threshold = no bonus), revealed via a multiplier callout + count-up animation on the win screen.

## Suggested order

Sessions run roughly in order: 1 → 2 → 3 → 4 → 5 → 5.1 → 5.2 → 5.3 → 5.4 → 6 → 7 → 8 → 9 → 9.1 → 10 → 10.1 → 10.2 → 10.2.1 → 10.2.2 → 10.2.3 → 10.2.4 → 10.2.5 → 10.2.6 → 10.2.7 → 10.2.8 → 10.2.9 → 10.2.10 → 10.3 → 10.4; each depends on prior sessions' output existing in the repo. Session 8 (polish), 9/9.1 (testing/deploy), and the 10.x special-powers/leaderboard/scoring sessions can be reordered or trimmed if time is short — everything through session 7 is a fully playable game without powers, the ten 10.2.x power sessions are independent of each other (any order, or subset, works once 10.2 is done), and 10.3/10.4 can be built in either order relative to each other (just reconcile which score value — pre- or post-multiplier — gets saved to the leaderboard, per 10.4's notes).
